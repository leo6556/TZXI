...
# with sq.connect('reminder.db') as con:
#     cur = con.cursor()
#
#     cur.execute('CREATE TABLE IF NOT EXISTS remind (user_id TEXT, information TEXT, something TEXT )')
...
def helping(update:Update, context:CallbackContext):
    ...
    texp = [
        ...
        '📍 Пожелания для улучшения сервиса и отладки ошибок можете направлять на контакт: @ten630531'
...
# ***************Ветка callback-ов для пассажиров*********
    if data == 'ending':
        ...
        context.bot.send_message(text=f'Пассажир подтвердил окончание поездки',
                                 chat_id=context.user_data[TEXTU])
        # # Временный видимый архив
        # with sq.connect('orders_database_archive.db') as con:
        #     yu = truly_text.split('Оплата')
        #     for_archive = yu[0] + '\n' + 'Заказ: пассажир подтвердил окончание поездки'
        #     cur = con.cursor()
        #
        #     cur.execute(f'INSERT INTO memory_p (main_data, user_id) VALUES ("{for_archive}",'
        #                 f'"{context.user_data[TEXTU]}{context.user_data[TEXTU2]}")')
    ...
    if data == 'ending2':
        ...
        if truly_text[4] == 'з':
            with sq.connect('orders_database_tomr.db') as con:
                cur = con.cursor()
                cur.execute(f'DELETE FROM ordersa WHERE price LIKE "%{truly_text[12:16]}"')
        context.bot.send_message(text=f'Водитель подтвердил окончание поездки',
                                 chat_id=context.user_data[TEXTU])
        # # Временный видимый архив
        # with sq.connect('orders_database_archive.db') as con:
        #     yu = truly_text.split('Оплата')
        #     for_archive = yu[0] + '\n' + 'Заказ: водитель подтвердил окончание поездки'
        #     cur = con.cursor()
        #
        #     cur.execute(f'INSERT INTO memory_p (main_data, user_id) VALUES ("{for_archive}",'
        #                 f'"{context.user_data[TEXTU]}{context.user_data[TEXTU2]}")')
...
def offer_p(update:Update, context:CallbackContext):
    ...
    if context.user_data[TIME1] in ('morning', 'daytime', 'evening'):
        ...
        if t[:2] in ('01','02','03','04','05'):
            t = '06:00'
        if t[:2] in ('21', '22', '23', '24', '00'):
            return to_late(update=update, context=context)
        ...
        if context.user_data[TIME1] == 'morning':
            ...
            if t[:2] in ('01','02','03','04','05','06','07','08','09','10','11'):
                t = '11:30'
            a = list_time.index(t)
            if context.user_data[DAY] == 'tomorrow':
                a = 12
    ...
    for i in n:
        ...
        if i[-3] == 'город':
            k = 'города (магнит)'
        if i[-3] == 'город' and context.user_data[WHERE1] == 'city':
            k = 'города'
    ...
    if len(te) < 42:
        ...
        if context.user_data[fps] == 7:
            ...
            with sq.connect('reminder.db') as con:
                ...
                cur.execute(f'INSERT INTO remind (user_id, information) VALUES ("{chat_id}", "{context.user_data[FROM1]},{context.user_data[TIME1]}")')

            update.callback_query.message.reply_text('К сожалению, водителей пока нет 😌 нам отправить вам'
...
def to_late(update=Update, context=CallbackContext):
    # update.callback_query.delete_message
    update.callback_query.message.reply_text('На сегодня уже поздно делать заказы (после 20:00) можете '
                              'найти поездку на завтра /drive')
    return ConversationHandler.END
